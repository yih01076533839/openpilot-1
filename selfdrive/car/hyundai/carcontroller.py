...
def process_hud_alert(enabled, fingerprint, visual_alert, left_lane,
  ...
  if not button_on:
    lane_visible = 0
  if left_lane and right_lane or sys_warning:  #HUD alert only display when LKAS status is active  ###바꿈
    if enabled or sys_warning:###바꿈
      sys_state = 3
    else:
      sys_state = 4
  elif left_lane:
  ...
  return sys_warning, sys_state, left_lane_warning, right_lane_warning   ###바꿈
...
class CarController():
  ...
  def update(self, enabled, CS, frame, actuators, pcm_cancel_cmd, visual_alert,
    ...
    can_sends.append(create_lkas11(self.packer, frame, self.car_fingerprint, apply_steer, lkas_active,
                                   CS.lkas11, sys_warning, sys_state, enabled, left_lane, right_lane,    ###바꿈
    ...
    if CS.out.cruiseState.standstill:
      ...
      elif self.last_lead_distance < CS.lead_distance and (frame - self.last_resume_frame) > 5:
